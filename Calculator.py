    ...
    try:
        choose_option = int(input(" \nEnter your operation through it's correcponding number: "))
            if choose_option == 1:
                ...
                num = int(input("Enter your number which you want to find a square: "))
                print(f"It's your square which you want: {square(num)}")
            elif choose_option == 10:
                num1 = int(input("Enter which number you want to find square root (like 144 = 12): "))
                num2_input = input("Enter for which root (e.g., 2 for square root, 3 for cube root) (default is 2): ")
                if num2_input.strip() == "":
                    result = Radical_value(num1) 
                else:
                    num2 = int(num2_input)
                    result = Radical_value(num1, num2)
                print(f"It's your square root value:{result} ")
    except ValueError:
        print("Just Choose suggested number please not anything else😅 ")
        exit()
